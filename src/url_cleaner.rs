...
use regex::Regex;
use url::Url;
// RETTELSE HER: Vi importerer DomainRule fra rules modulet
use crate::Config;
        ...
        for (k, v) in current_query {
            ...
            if let Some(r) = rule {
                if let Some(ref keep) = r.keep_only {
                    // RETTELSE: Tilføjet .as_str() for at hjælpe compileren
                    if !keep.iter().any(|allowed| allowed.as_str().to_lowercase() == dominated) {
                        should_remove = true;
                    } else {
                        is_whitelisted = true;
                    }
                    // RETTELSE: Tilføjet .as_str() for at hjælpe compileren
                } else if r.params.iter().any(|p| p.as_str().to_lowercase() == dominated) {
                    should_remove = true;
                }
            }
            ...
            if !should_remove && !is_whitelisted {
                // RETTELSE: Tilføjet .as_str() for at hjælpe compileren
                if self.config.global_params.iter().any(|p| p.as_str().to_lowercase() == dominated) {
                    should_remove = true;
